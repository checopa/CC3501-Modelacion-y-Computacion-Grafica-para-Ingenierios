...
def elipse(radio1, radio2, altura, pos=Vector(0, 0, 0)):
    """
    @ elipse: Genera los vertices para una elipse con base centrada en pos, con radio mayor r2 y radio menor r1
    """
    # delta angular, precision
    dang = 2 * pi / 50

    p1 = sumar(Vector(0, 0, 0), pos)  # centro base
    p2 = sumar(VectorCilindricas(radio1, 0, 0), pos)  # punto inicial base
    p4 = sumar(Vector(0, 0, altura), pos)  # centro tapa
    p5 = sumar(VectorCilindricas(radio1, 0, altura), pos)  # punto inicial tapa

    theta = 0
    while theta <= 2 * pi+dang:
        p3 = sumar(Vector(radio1*cos(theta),radio2*sin(theta),0), pos)  # siguiente punto en la iteracion
        p6 = sumar(Vector(radio1*cos(theta),radio2*sin(theta),altura), pos)
